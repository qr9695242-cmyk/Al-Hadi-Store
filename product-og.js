...
module.exports = (req, res) => {
  try {
    ...
    if (product) {
      ...
      const outOfStock = product.stockStatus === 'out' || (product.stockQty != null && Number(product.stockQty) <= 0);

      const productJsonLd = JSON.stringify({
        '@context': 'https://schema.org',
        '@type': 'Product',
        name: product.name,
        image: [absoluteUrl(firstImage) || (SITE_URL + '/assets/og-image.png')],
        description: rawDesc,
        sku: product.productCode || product.id,
        offers: {
          '@type': 'Offer',
          url: SITE_URL + '/?p=' + encodeURIComponent(id),
          priceCurrency: 'PKR',
          price: product.price,
      ...
      html = html
        ...
        .replace('</head>', `<script type="application/ld+json">${productJsonLd}</script>\n</head>`);
    }
