...
/* ---------- coordinated first paint ----------
   Pehle: embedded catalog (BASE_PRODUCTS) turant ready ho jata tha aur
   akela render ho jata tha; Firestore custom products (jin mein "New" aur
   "Flash Sale" wale items hote hain) network se der se aate the, is liye
   woh baad mein "pop in" ho kar dobara render hote — purana product pehle,
   naye/flash sale baad mein.
   Fix: dono sources (base + custom) ready hone tak — ya chhota fallback
   timeout guzarne tak — pehla render rok kar rakhte hain, taake purane,
   naye aur flash sale sab products ek hi baar mein, ek sath show hon. */
let baseProductsReady = false;
let firstCustomSyncSettled = false; // true jab pata chal jaye kya dikhana hai: real data, "no data", ya timeout
let initialCatalogRendered = false;
const FIRST_SYNC_FALLBACK_MS = 2500; // Firestore slow/offline ho to pehla render hamesha ke liye na roken
...
function renderCatalog(){
  ...
  renderAdminProductList();
  openProductFromURL();
  initialCatalogRendered = true;
}
function maybeRenderInitialCatalog(){
  if(initialCatalogRendered) return;
  if(baseProductsReady && firstCustomSyncSettled) renderCatalog();
}
...
function watchCustomProducts(){
  if(typeof firebase === 'undefined' || !firebase.firestore){
    ...
    firstCustomSyncSettled = true;
    maybeRenderInitialCatalog();
    return;
  }
  const firstSyncFallback = setTimeout(function(){
    firstCustomSyncSettled = true;
    maybeRenderInitialCatalog();
  ...
  firebase.firestore().collection('products').onSnapshot(function(snap){
    ...
    firstCustomSyncSettled = true;
    if(!initialCatalogRendered) maybeRenderInitialCatalog();
    else renderCatalog(); // pehla paint ho chuka — ye ab live update hai (admin ne kahin aur se product add/edit/delete kiya)
  }, function(err){
